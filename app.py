...
def get_custom_fortune(dong, ho, type_dict):
    ...
    weather_pools = {
        "맑음": [
            f"오늘 외부의 청명한 양기(陽氣)가 <b>{dong} {ho}호</b> 터의 가장 깊은 곳까지 강하게 쏟아져 들어오고 있습니다. 풍수적으로 맑은 양광(빛)은 막힌 운을 뚫어주고 명예와 결실을 맺게 하는 최고의 에너지입니다. 아직 빈 공간임에도 맑고 힘찬 생기가 터를 감싸고 있어, 훗날 귀하께서 입주하셨을 때 집안에 웃음소리가 마르지 않고 뜻밖의 횡재수와 귀인이 끊임없이 찾아들 대길(大吉)의 기운을 머금고 있습니다.",
            f"구름 한 점 없는 맑은 하늘의 순수한 기운이 <b>{dong} {ho}호</b> 터에 온전히 내리쬐며 자리를 잡고 있는 날입니다. 터가 품은 본연의 생명력이 극대화되는 매우 길한 시기입니다. 훗날 이 공간에서 새롭게 시작하는 가족들의 모든 일들이 막힘없이 풀려나가고, 건강운과 학업운이 크게 상승하게 되는 천혜의 명당으로 거듭나게 됩니다."
        ],
        "흐림": [
            f"하늘에 드리운 묵직한 구름처럼, 현재 <b>{dong} {ho}호</b>의 터는 들뜬 기운을 가라앉히고 숨을 깊게 고르며 거대한 지운(地運)을 단단하게 응축하고 있는 중입니다. 풍수에서는 이렇게 기운이 갈무리되는 터를 재물이 흩어지지 않는 '금고'의 형상으로 봅니다. 훗날 이곳에 입주하시게 되면 밖으로 허무하게 샐 뻔했던 자금들이 완벽히 차단되고, 재물이 차곡차곡 쌓여 크게 쓰일 튼튼한 재물창고가 되어줄 것입니다.",
            f"햇빛이 잠시 가려지고 대지가 차분해진 오늘, <b>{dong} {ho}호</b>의 터는 바깥의 소란스러운 에너지를 차단하고 스스로 에너지를 모으는 '저장과 잉태'의 시기를 맞이했습니다. 이 터가 품고 있는 묵직한 지운은 훗날 예기치 못한 금전적 보상이나 큰 계약을 성사시킬 수 있는 단단하고 흔들림 없는 기반이 되어줄 것입니다."
        ],
        "비": [
            f"하늘에서 내리는 수(水)의 기운이 <b>{dong} {ho}호</b> 터 주변에 혹여나 맴돌고 있을지 모를 묵은 액운과 정체된 기운을 시원하게 씻어내리고 있습니다. 명리학과 풍수지리에서 맑은 물은 곧 재물(財物)이 흘러들어오는 통로를 의미합니다. 입주를 앞두고 터가 깨끗하게 정화되고 있으니, 이사 후에는 그동안 꽉 막혀 답답했던 자금 문제나 인간관계의 꼬임들이 시원하게 뚫리게 될 훌륭한 명당의 기운입니다."
        ]
    }
    site_energy = random.choice(weather_pools.get(weather, weather_pools["맑음"]))

    # ----------------------------------------
    # [블록 2] 평수 기반 완벽 타겟팅 (신혼/1인가구 vs 가족 vs 자산가)
    # ----------------------------------------
    if "59" in unit_type: 
        vibe_title = "🌱 실속과 새로운 시작의 기운"
        fortune_pools = [
            "이 터는 단단한 흙을 뚫고 힘차게 돋아나는 '새싹(生動)'의 기운을 품고 있습니다. 이제 막 사랑하는 사람과 새로운 가정을 꾸린 신혼부부나, 온전한 독립을 이룬 1인 가구, 혹은 무거운 짐을 내려놓고 제2의 여유로운 인생을 시작하는 분들에게 더할 나위 없는 명당입니다. 불필요한 겉치레를 덜어낸 알찬 공간인 만큼, 오늘은 작은 지출을 통제하는 것이 훗날 큰 종잣돈으로 돌아오는 형국입니다. 오늘 저녁 사랑하는 이와 따뜻한 식사를 나누며 화합하면 엉뚱하게 돈이 새는 것을 완벽히 막아줄 것입니다.",
            "작지만 그 속이 아주 알차게 여문 '단단한 금고'의 기운이 서려 있습니다. 1~2인 가구의 단란함과 실속을 채우기에 이보다 좋은 터가 없습니다. 남들의 화려한 겉모습이나 소비에 흔들리지 않고 귀하만의 뚝심으로 밀고 나갈 때 비로소 묵직한 재물이 터에 고이게 됩니다. 오늘은 충동구매의 유혹이 강하게 들어오니 장바구니를 비우고 하루를 조용히 관망하십시오. 오늘 절약한 그 돈이 직장이나 사업에서 두 배의 가치를 지닌 귀한 소식으로 돌아옵니다."
        ]
    elif "110" in unit_type or "114" in unit_type or "104" in unit_type: 
        vibe_title = "⛰️ 성취와 대인(大人)의 기운"
        fortune_pools = [
            "이 터는 높은 곳에서 세상을 여유롭게 굽어보는 장엄한 '태산(泰山)'의 기운을 품고 있습니다. 치열한 시기를 지나 어느 정도 경제적 성취를 이룩하셨거나, 자녀들을 어엿하게 장성시킨 분들의 높은 품위와 권위를 굳건히 지켜주는 귀한 자리입니다. 이런 대인의 터에 입주하실 분이라면, 이제는 아등바등 얕은 수나 자잘한 푼돈 지출에 너무 연연하실 필요가 없습니다. 오늘은 주변 지인들이나 후배들에게 여유롭게 식사를 대접하며 넉넉한 덕(德)을 베푸십시오. 그 베풂이 훗날 더 큰 명예와 기가 막힌 투자 호재를 물어다 줄 것입니다.",
            "수많은 배들이 평화롭게 정박할 수 있는 드넓고 고요한 '심해(深海)'의 묵직한 기운을 품은 터입니다. 귀하가 평생에 걸쳐 쌓아 올린 자산과 명예를 흔들림 없이 지켜주며, 나아가 거주자의 건강과 장수를 기약하는 대인(大人)의 명당입니다. 오늘 하루는 사소한 오해나 주변 사람들의 가벼운 언쟁에 휘말리지 마시고, 바다처럼 넓은 아량으로 품어주십시오. 귀하의 여유로운 태도가 곧 명예를 더욱 높여주고, 새로운 혜안을 제공할 귀인들을 끌어모으는 강력한 자석이 됩니다."
        ]
    else: 
        vibe_title = "🌳 안정과 가족 화목의 기운"
        fortune_pools = [
            "이 터는 만물을 품고 기르는 '대지(土)'의 기운으로, 3~4인 가족의 중추를 든든하게 책임지고 화목함을 이끌어가는 데 최적화된 명당입니다. 한창 자라나는 자녀들의 학업운과 가족의 북적이는 생기를 지탱해 주는 힘이 탁월합니다. 오늘 귀하의 운세를 여는 마스터키는 바로 '가족의 웃음'입니다. 바깥에서 무리하고 공격적인 투자를 모색하기보다 자녀나 배우자에게 따뜻한 칭찬 한마디를 건네보십시오. 집안에 긍정적인 양기(陽氣)가 차오르며 뜻밖의 경제적 호재를 끌어당기게 될 것입니다.",
            "가정의 경제를 책임지며 힘찬 도약을 폭발적으로 이루어가는 분들에게 최고의 안정감을 주는 '거목(巨木)'의 터입니다. 외부의 거센 풍파에도 집안을 안전하게 지켜주는 단단한 방패막이 되어줍니다. 오늘은 늦은 외부의 술자리나 약속보다는 일찍 귀가하여 거실에서 가족들과 소박하고 깊은 대화를 나누는 것이 운을 크게 트이게 합니다. 가족 간의 결속력이 다져질수록 터에 남아있던 음기(陰氣)가 씻은 듯 물러가고 거주자의 건강과 재산이 크게 불어납니다."
        ]
